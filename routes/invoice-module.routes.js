...
// ----------------------------------------------------
// 10. DELETE /api/admin/customers/:id (Admin Customer Delete)
// ----------------------------------------------------
router.delete("/admin/customers/:id", adminAuth, async (req, res) => {
  try {
    ...
    // Check user
    const [usr] = await sql`SELECT * FROM users WHERE user_id = ${userId} LIMIT 1`;
    if (!usr) return fail(res, "Customer not found.", 404);

    // Prevent deletion of 1st Associate / Admin account
    if (Number(userId) === 1 || usr.member_id === 'MMR-ASC-0001' || usr.user_type === 'Admin' || usr.role === 'Admin') {
      return fail(res, "The 1st Associate / Admin account (ID #1) is a protected root account and cannot be deleted.", 403);
    }

    // Release any booked plots by this customer
    const customerBookings = await sql`SELECT booking_id, plot_id FROM bookings WHERE customer_user_id = ${userId}`;
    for (const bk of customerBookings) {
      await sql`DELETE FROM invoices WHERE booking_id = ${bk.booking_id}`;
      await sql`DELETE FROM booking_invoices WHERE booking_id = ${bk.booking_id}`;
      await sql`DELETE FROM booking_payment_records WHERE booking_id = ${bk.booking_id}`;
      await sql`DELETE FROM bookings WHERE booking_id = ${bk.booking_id}`;
      if (bk.plot_id) {
        await sql`UPDATE plots SET current_status = 'Available', is_booked = FALSE WHERE plot_id = ${bk.plot_id}`;
      }
    }

    // Delete user documents & record
    await sql`DELETE FROM user_documents WHERE user_id = ${userId}`;
    await sql`DELETE FROM referral_registrations WHERE referred_user_id = ${userId}`;
    await sql`DELETE FROM users WHERE user_id = ${userId}`;

    return ok(res, {}, `Customer "${usr.full_name}" (ID #${userId}) deleted successfully.`);
  } catch (error) {
...
// ----------------------------------------------------
// 11. DELETE /api/admin/associates/:id (Admin Associate Delete)
// ----------------------------------------------------
router.delete("/admin/associates/:id", adminAuth, async (req, res) => {
  try {
    const userId = req.params.id;

    // Check associate
    const [usr] = await sql`SELECT * FROM users WHERE user_id = ${userId} LIMIT 1`;
    if (!usr) return fail(res, "Associate not found.", 404);

    // Prevent deletion of 1st Associate / Admin account
    if (Number(userId) === 1 || usr.member_id === 'MMR-ASC-0001' || usr.user_type === 'Admin' || usr.role === 'Admin' || (!usr.sponsor_user_id && Number(userId) === 1)) {
      return fail(res, "The 1st Associate / Admin account (ID #1) is a protected root account and cannot be deleted.", 403);
    }

    // Delete associate referral hierarchy and records
    await sql`DELETE FROM associate_network_closure WHERE ancestor_user_id = ${userId} OR descendant_user_id = ${userId}`;
    await sql`DELETE FROM associate_profiles WHERE associate_user_id = ${userId}`;
    await sql`DELETE FROM referral_registrations WHERE associate_user_id = ${userId} OR referred_user_id = ${userId}`;
    await sql`DELETE FROM commission_transactions WHERE associate_user_id = ${userId}`;
    await sql`DELETE FROM users WHERE user_id = ${userId}`;

    return ok(res, {}, `Associate "${usr.full_name}" (ID #${userId}) deleted successfully.`);
  } catch (error) {
    console.error("DELETE /api/admin/associates/:id error:", error);
    return fail(res, error.message);
  }
});
